...
def fsave_data(filename, batch_source_words, batch_source_ids, batch_ref_words, batch_ref_ids):
    ...
    files = {length: open(filename + "." + str(length), 'a', encoding='utf-8') for length in max_lengths }
...
def preprocess_data(filename):
    ...
    # data.columns = ['1','2']
    # data['1'].apply(lambda x: batch_source_sentences.append(str(x)))
    # data['2'].apply(lambda x: batch_ref_sentences.append(str(x)))
...
if __name__ == '__main__':
    main()
    # print (generate_length_index([5,10,15,20,30,40,50]))
